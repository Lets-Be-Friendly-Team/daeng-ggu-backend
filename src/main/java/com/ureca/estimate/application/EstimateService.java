...
public class EstimateService {
    ...
    public void makeEstimate(EstimateDto.Request request, List<MultipartFile> estimateImgList) {
        ...
        Estimate estimate = Estimate.builder()
                .designer(designerRepository.findByDesignerId(request.getDesignerId()).get())
        ...
        for(MultipartFile estimateImg : estimateImgList) {
            String estimate_img_url = s3Service.uploadFileImage(estimateImg,"estimate","estimate");
            EstimateImage estimateImage = EstimateImage.builder().estimate_img_url(estimate_img_url).estimate(estimate).build();
    ...
    public List<EstimateDto.Response> selectCustomerEstimate(Long customerId) {
        ...
        for(Pet pet : pets) {
            ...
            EstimateDto.Response response = EstimateDto.Response.builder()
                    .petId(pet.getPetId())
                    ...
                    .estimateList(estimateRepository.findAllByRequest(request).stream()
                            .map(estimate -> EstimateDtoDetail.builder()
                                    ...
                                    .estimatePrice(estimate.getGrooming_fee())
    ...
    public List<EstimateDto.Response> getPreviousEstimatesByDesigner(Long designerId) {
        ...
        List<EstimateDto.Response> estimateList =  estimates.stream()
                .map(estimate -> EstimateDto.Response.builder()
                        .petId(estimate.getRequest().getPet().getPetId())
                        .petName(estimate.getRequest().getPet().getPetName())
                        .petImageUrl(estimate.getRequest().getPet().getPetImgUrl())
                        .desiredServiceCode(estimate.getRequest().getDesired_service_code())
                        .lastGrommingDate(estimate.getRequest().getLast_grooming_date())
                        .desiredDate1(estimate.getRequest().getDesired_date1())
                        .desiredDate2(estimate.getRequest().getDesired_date2())
                        .desiredDate3(estimate.getRequest().getDesired_date3())
                        .desiredRegion(estimate.getRequest().getDesired_region())
                        .isDelivery(estimate.getRequest().getIs_delivery())
                        .isMonitoringIncluded(estimate.getRequest().getIs_monitoringIncluded())
                        .additionalRequest(estimate.getRequest().getAdditional_request())
                        .build())
                .collect(Collectors.toList());
